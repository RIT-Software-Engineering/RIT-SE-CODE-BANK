                    ...
                    for(let x of props.semesterData){
                        if(tracker.includes(x.semester_id)){
                            ...
                            tracker3.push(setWeeks(eachWeekOfInterval({
                                // temporary solution, props.semesterData is an array so looking up by x.semester_id often gives the wrong index
                                // having a semester_id greater than length of semesterData prompts the error catch 
                                start: new Date(props.semesterData[x.semester_id - x.semester_id].start_date),
                                end: new Date(props.semesterData[x.semester_id - x.semester_id].end_date)
                            })))
                        }
                    }
