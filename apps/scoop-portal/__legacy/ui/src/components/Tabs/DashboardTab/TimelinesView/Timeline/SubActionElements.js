...
export default function SubActionElements(props) {
    const sortedActions = _.sortBy(props.subActions || [], ["due_date", "start_date", "action_title"]);
    ...
    sortedActions.forEach((action, idx) => {
        ...
        const trigger = <button
            key={idx}
        >
            {<div className="action-bar-text" title={action.action_title}>CHILD!!! {action.action_title}</div>}
        </button>
