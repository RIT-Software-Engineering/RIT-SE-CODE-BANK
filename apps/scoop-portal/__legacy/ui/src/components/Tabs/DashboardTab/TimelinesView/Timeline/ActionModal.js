      ...
      for (let x = 0; x < formDataInputs.length; x++) {
        ...
        if (formDataInputs[x].type === "radio") {
          if (isRequiredAndEmpty(x) && !errorsSet.has(formDataInputs[x].name)) {
            console.log('Radio validation error:', {
              index: x,
              element: formDataInputs[x],
              name: formDataInputs[x].name,
              type: formDataInputs[x].type,
              value: formDataInputs[x].value,
              required: formDataInputs[x].required
            });
            errors.push(
          ...
          if (isRequiredAndEmpty(x)) {
            ...
            if (!formDataInputs[x].name) {
              errors.push(`Form field #${x + 1} is missing a name attribute and cannot be empty`);
            } else {
