...
export default function DatabaseTableEditor(props) {
  ...
  const handleSubmit = async function (e) {
    console.log('DatabaseTableEditor handleSubmit called');
    console.log('Current form data:', formData);
    
    const dataToSubmit = !!props.preSubmit
      ? await props.preSubmit(formData)
      : formData;
    ...
    try {
      const response = await SecureFetch(submitRoute, {
        method: "post",
        body: body,
      });
  ...
  const handleChange = (e, { name, value, checked, isActiveField }) => {
    console.log('DatabaseTableEditor handleChange:', { name, value, checked, isActiveField });
    ...
    if (field?.onChange) {
      console.log('Calling field onChange handler for:', name);
      field.onChange(e, { name, value, checked });
    }
    ...
    if (newFormData) {
      console.log('Setting form data from preChange:', newFormData);
      ...
      console.log('Setting form data directly:', {
        ...formData,
    ...
    if (!field.hidden) {
      switch (field.type) {
        ...
        case "phoneInput":
          ...
          break;
          case "date":
            const rawDateValue = formData[field.name];
            const dateValue =
              rawDateValue && !isNaN(Date.parse(rawDateValue))
                ? new Date(rawDateValue).toISOString().substring(0, 10)
                : "";
