...
const mockUser = (req, res, next) => {
    ...
    if (process.env.NODE_ENV === 'development') {
        // Check if cookies exist and have necessary properties
        if (req.cookies && req.cookies.system_id) {
            req.user = {
            ...
            // Set a default user for development if no cookies are present
            req.user = {
                system_id: 'dev_user',
                fname: 'Development',
                lname: 'User',
    ...
    if (req.cookies && req.cookies.mock_system_id && testIsAdmin(req)) {
        req.user = {
...
const testIsAdmin = (req) => {
    return req.user && req.user.type === USERTYPES.ADMIN;
}

const testIsCoach = (req) => {
    return req.user && req.user.type === USERTYPES.COACH;
}
