...
const dummy_data_path = "./server/database/test_data_scoop";
...
function populateDummyData() {
    return new Promise((resolve, reject) => {
        fs.readdir(dummy_data_path, (err, files) => {
            ...
            files
                .filter((file) => file.toString() != "fill_test_data_scoop.sql")
                .forEach((file) => {
...
async function initializeDatabase() {
    try {
        Logger.log("Starting database redeploy (scoop)");
        ...
        Logger.log("Done redeploying scoop database");
    } catch (error) {
        console.error(error);
    }
}
