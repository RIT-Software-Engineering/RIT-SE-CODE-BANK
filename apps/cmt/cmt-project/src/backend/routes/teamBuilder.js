...
module.exports = function makeTeamBuilderRouter(prisma) {
  ...
  // ---------- UPLOAD ROSTER CSV (only this prof's course) ----------
  // POST /api/team-builder/courses/:courseId/roster
  router.post(
  ...
  // ---------- PUBLISH / UNPUBLISH / ARCHIVE / EDIT / MOVE ----------
  router.patch("/teamsets/:id/publish", async (req, res) => {
    try {
      ...
      const set = await prisma.TBTeamSet.update({
        ...
        include: {
          teams: {
            include: { members: { include: { enrollment: true } } },
          },
        },
      });
      res.json(set);
