...
function getChildActions(action) {
    if (action?.childActionsWithContexts)
        return action.childActionsWithContexts.map(
            actionWithContexts => actionWithContexts.processedAction
        );
...
export function SimpleActionRenderer(props) {
  const metadata = getActionMetadata(props.action);
      ...
      <Card.Body>
          ...
          <div>
            ...
            {(metadata.outputs||[]).map((output, index) => {
                return (<div key={output.key || output.name || index}>
...
export function ComplexActionRenderer(props) {
  const childActions = getChildActions(props.action);
