  ...
  const onSubmitCandidate = async (data) => {
    try {
      ...
      if (data.graduateStatus === "GRADUATE") {
        ...
        if (isNaN(year)) {
          throw new Error(
            "Invalid year level selected. Please choose a valid year."
          );
        }
      } else {
              ...
              <>
                <fieldset className="space-y-4">
                  <div>
                    ...
                    {errors.graduateStatus && (
                      <p className="text-red-500 text-xs mt-1">
                        {errors.graduateStatus.message}
                      </p>
                    )}
                  </div>
                ...
                <fieldset>
                  <legend className={formLegend}>
                    Are you currently or have you ever been a Course Assistant?{" "}
                ...
                {isEmployeeValue === "yes" && (
                  <fieldset>
                            ...
                            {...register("coursesWorked", {
                              validate: (value) =>
                                ...
                                (!value || value.length === 0)
                                  ? "Select at least one course you've worked for."
                                  : true,
                            })}
                            value={course.courseCode}
                            className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    ...
                    {errors.coursesWorked && (
                      <p className="text-red-500 text-xs mt-1">
                        {errors.coursesWorked.message}
                      </p>
                    )}
                  </fieldset>
                )}
