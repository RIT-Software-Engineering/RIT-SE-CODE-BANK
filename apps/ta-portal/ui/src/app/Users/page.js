...
export default function Admin() {
    ...
    const handlePromoteToAdmin = async (user) => {
        try {
            ...
            if(!user?.employer) {
                console.error("User is not an employer. Cannot promote to Admin:", user);
                return;
            }
    ...
    if (isLoading) {
        return (
            ...
            <div className="w-8 h-8 border-4 border-dashed rounded-full animate-spin border-rit- mx-auto"></div>
        ...
        .map((role) => {
            ...
            if (role === 'EMPLOYEE') {
                ...
                const status = user?.candidate?.employees?.[0]?.employeeStatus || 'UNKNOWN';
